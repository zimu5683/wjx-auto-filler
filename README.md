...
| 左右双栏映射 | 左栏**目标字段**（题号 `1`/`q1` 或题干关键词），右栏**填充内容**；**没有条数上限**，列表随条数增长并显示条数（如「字段映射（5 条）」）；**支持超量预填**（一份配置可适配多份问卷，问卷里没有的字段会被**跳过**并在结果里显式列出） |
...
| 人机验证检测 | 检测到需要人机验证时**直接进入验证页**（不自动点击、不自动解题）；**提前提醒**（默认 10 分钟）；到点被拦截推送通知 |
...
- **不做设备重启后自动恢复**；也**不做自主人机验证**（不自动点击、不自动解题）：检测到需要验证时自动进入验证页，
  由你本人完成一次；取消后不会反复自动弹出。
...
1. **解析问卷** → 顶部状态条显示开放状态：「开放时间未知：点「解析问卷」获取」/「尚未开放，将于 2026-09-23 09:00 开放」/「已开放，可提交」
   ...
   **每次解析都会自动把开放时间填入下面的「问卷开放时间」**（**尚未开放的问卷同样会填入**，状态条显示「尚未开放，将于 … 开放」；
   时间一律按**北京时间 GMT+08:00**，不受设备时区影响）；
   解析不到时**保留你原来的填写**并提示「未解析到开放时间，请手动填写」（**不静默清空**）
2. **保存模板** → 定时任务必须绑定一份模板
3. 填写 **问卷开放时间**（格式 `2026-09-23 09:00`）与 **提前提醒（分钟）**（默认 **10**，`0` = 不提前提醒）；
   可选 **到点响铃提醒（默认开）** / **到点震动提醒（默认开）**（两者都关仍推送高优先级全屏通知）
...
### 人机验证检测（检测到即进入验证页，不破解）

- **配置页不再显示「该问卷涉及人机验证」横幅**（v1.0.5 按用户要求移除）；**检测能力保留**：
  提交返回 `7`/`22` 后 **App 自动进入验证界面**（不自动点击、不自动解题，验证由你本人完成一次；
  取消后不会反复自动弹出，可用「人工验证后重试」手动重试）
- 可设 **提前提醒（默认 10 分钟）**：开放前推送「问卷即将开放 … 请留意人机验证」
- 到点被拦截：推送高优先级通知并**尝试全屏拉起验证页**；回到 App 后收到 `E_CAPTCHA` 会**自动进入验证界面**
  （不自动点击、不自动解题，验证由你本人完成；取消后不会反复自动弹出，可用按钮手动重试）—— 流程见 [docs/USAGE.md](docs/USAGE.md) 7.1
...
- **定时自动提交**会在开启期间运行**前台常驻服务**并显示常驻通知（仅此功能使用）；关闭开关即停止。
  通知内容只包含模板名、开放时间与提交结果摘要，不含答案明细
