...
| 验证码兜底 | `useAliVerify=1` 问卷的**常规路径**（服务端返回码 7 / 22 时）：**由你本人**在内置页面完成一次验证，App 用同一会话 + 令牌继续走纯接口提交（不填表、不点提交） |
...
| `https://v.wjx.cn/vm/P2M09FG.aspx` | `0` | `ktimes=0` | 正文 `22`（**裸业务码，无 `〒`**）→ 业务码 **22**（风控指纹） |
| 同上（同一问卷，只改 `ktimes`） | `0` | `ktimes>0` 且**不补发**校验类字段 | 正文 `10〒/wjx/join/complete.aspx?activityid=P2M09FG&joinid=…` → **提交成功** |
...
- `useAliVerify=1` 的问卷会被服务端强制要求安全校验（业务码 `7`）→ 走下面的人工验证流程，
  **对这类问卷兜底是常规路径**；
- App 按实测约束发**最小请求**：`ktimes = max(1, 页面值)`，且**未取得验证令牌时绝不携带**
...
2. 被页面门控（`useAliVerify=1`）或响应码 `7`/`22` 拒绝 → 结果区显示「需验证」与 **人工验证后重试** 按钮；
