 ...
 * 问卷星实现（T16，Lead 实测依据）：
 *
 * 1. **时间戳优先**：页面 JS 变量 `var qBeginDate="1790040856347"`（epoch millis）在**所有问卷页**都有；
 *    已开放/未开放都用同一字段，**用时间戳比较判定，不要猜文案**。
 * 2. **文案兜底**：`<div id='divstarttime' left='N'>很抱歉，此问卷将于2026-09-23 09:33（北京时间）开放…`
 * 3. 解析失败 → [OpenTime.Unknown]（不拦截）。
 *
