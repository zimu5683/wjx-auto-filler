...
| G4 | 未匹配字段**不得静默丢弃**：可跳过，但必须**显式可见** | 未匹配字段 → 提交仍成功，但结果区列出「已跳过 N 个字段：…」；歧义/冲突/取值非法/全部被跳过 → `E_UNMATCHED` 报错并高亮 |
...
6. **解析失败也必须收敛 UI 状态（v1.0.7，根因类别）**：失败分支**不得沿用上一次成功解析的结果**——`state.survey`、状态条、题目清单、开放时间输入框都要按错误码置为明确值或 `UNKNOWN`，并**与成功分支一样调用状态渲染**。反例：未开放问卷解析失败后状态条仍显示「已开放，可提交」（读的是上一份问卷的旧模型），且开放时间没自动填入。
...
| 匹配歧义/冲突/取值非法/全部被跳过 | `E_UNMATCHED` | 匹配到多个题目（1,2）／多个字段指向同一题／取值不在选项中／全部字段被跳过 | 跳回映射页并高亮 |
| 未匹配字段（**可跳过**） | —（不报错） | 结果区显示「已跳过 N 个字段：…」 | 提供回到映射页入口；**绝不静默**（契约 §7.4） |
...
> **开放时间自动填入（v1.0.7）**：解析未开放问卷时 `fetch` 返回 `E_NOT_OPEN`，其 `WjxException.openAtMillis`（契约 §2.1）**必须**自动填入定时任务的开放时间输入框；解析成功时用 `SurveyModel.openAtMillis` 覆盖输入框；解析不到（`null`）则保留原值并提示「未解析到开放时间」。
...
> **开放时间的来源（T16 实测修正，别再踩）**：`openAtMillis` 由 `WjxTimeAdapter` 按「**未开放文案 → `left`+`nowTime` → `qBeginDate`/`BeginDate` 兜底 → Unknown**」解析（契约 §2.3）。**`qBeginDate` 是问卷开始/创建时间，不是定时开放时间**——按它比较会把未开放判成已开放，进而题目为空误报 `E_PARSE`。`Unknown` 时 `isOpen` 返回 `true`（不拦截手动提交）。
...
| 2026-09-22 | **v1.0.5 用户驱动反转**：未匹配字段由「一律失败」改为「可跳过但必须显式列出」（契约 §7.2/§7.4）；G4 与 §4.2 错误表同步；模板 upsert 改为按名字（契约 §4.5） | 用户 / Lead / architect |
| 2026-09-22 | T16 实测修正：开放时间解析优先级改为「文案 → `left`+`nowTime` → 时间戳兜底」（`qBeginDate` 只是创建时间）；§13.1 加防复发说明 | api-debug（证据）/ architect |
| 2026-09-22 | **v1.0.7**：§4.1 新增第 6 条原则「解析失败也必须收敛 UI 状态」；§13.1 补开放时间自动填入口径（`E_NOT_OPEN` → `WjxException.openAtMillis`） | Lead / architect |
