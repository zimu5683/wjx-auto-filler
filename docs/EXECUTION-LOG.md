...
## 4.6 第三轮：v1.0.5（用户真机实测驱动的三处修改）

用户装 v1.0.4 实测后提三处修改，**其中一处推翻了我在第一轮定下的设计决策**。

| # | 用户要求 | 根因 / 处置 |
|---|---|---|
| 1 | 删掉「该问卷涉及人机验证」横幅 | 布局整块 + `renderCaptchaBanner()` + 监听 + 6 条字符串全删；**保留**兜底流程（返回 7/22 **直接进入验证页**）与结果区重试按钮 |
| 2 | 删导入/导出模板；修「保存模板只能存一个」；加删除模板 | **真 bug**：`saveTemplate()` 用 `state.current.copy(name=…)` **保留了原 id**，而 `upsertTemplate()` **按 id 匹配** → 起什么名字都命中同一条 → 永远「共 1 个模板」。改为**按名字 upsert**（同名覆盖保留 id、新名新 id）+ `TemplateLibrary` 纯函数 + 删除模板（二次确认） |
| 3 | 字段映射支持**超量预填** | **推翻第一轮决策**：原为「歧义与未命中都算失败，绝不静默丢弃，任一失败整次中止」。现改为「未匹配到任何题目 → 跳过不阻断」；歧义/同题/取值错/全部被跳过 → **仍失败**。**跳过必须可见**（结果区显式列出 + 左栏高亮），以此承载"不静默丢弃"的精神 |

### 本轮又一次证据驱动的修正（Lead 的错误被下属顶回）

**我给全队的「事实 #1」是错的**：我说 `BeginDate` 是问卷**开放时间**。api-debug 用三源对照推翻：

| 来源 | 值（+08） |
|---|---|
| `qBeginDate="1790040856347"` | 2026-09-22 **09:34**（已过去 = **创建/开始时间**） |
| 未开放文案 | 2026-09-23 **09:33** |
| `nowTime` + `left` | 2026-09-23 **09:33:00** |

差 **23.98 小时**。按我的说法会把未开放问卷判成"已开放" → 题目为空 → **误报 E_PARSE**（正是我们要修的那个 bug）。
**修正后优先级**：未开放文案 → `left`+`nowTime` → `qBeginDate/BeginDate` 兜底 → Unknown（不拦截）。
**Lead 复核**：`git show v1.0.4:...WjxTimeAdapter.kt` 确认**已发布的 v1.0.4 里就是修正版**，未带此 bug 出货；契约 §2.3 已更新并保留防误报回归用例。
